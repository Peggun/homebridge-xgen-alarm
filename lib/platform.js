...
class XGenAlarmPlatform {
  ...
  setupSecurityService(accessory) {
    ...
    // This panel's web UI only exposes Away / Stay / Disarm buttons — no
    // confirmed command for Night, so don't offer it as something HomeKit
    // can *set* (we still report NIGHT_ARM as a possible *current* state,
    // since the panel can apparently reach it via the physical keypad).
   ...
   * (e.g. the panel staying unreachable, or a session that keeps
   * expiring) is logged at error level again only every 20th occurrence —
   * in between, it drops to debug level (visible with `homebridge -D`),
   * so a sustained outage produces one clear alert instead of one log
   * line per poll forever.
   * @param {Error} err
   */
