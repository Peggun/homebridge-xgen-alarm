...
class XGenAlarmPlatform {
    ...
    modes.forEach((mode) => {
      switch (mode.toLowerCase()) {
        ...
        case 'away':
          ...
          break;
        // Night isn't offered: there's no confirmed /user/keyfunction.cgi
        // fnum for requesting it (see the "Arm / Disarm" section of
        // xgenClient.js's class doc comment — only Away/Stay/Disarm have
        // confirmed fnums), so this plugin has no way to actually arm
        // Night even if HomeKit asked for it. It also isn't reported as
        // a distinct *current* state any more — see deriveState() in
        // xgenClient.js, which folds the panel's Night sub-mode into
        // plain STAY_ARM.
        case 'off':
        case 'disarm':
  ...
  async handleSetChimeState(value) {
    const { Characteristic, HapStatusError, HAPStatus } = this.api.hap;
    let action;
    
    switch (value) {
      case Characteristic.SecuritySystemTargetState.CHIME_ON:
        action = 'CHIME_ON';
        break;
      case Characteristic.SecuritySystemTargetState.CHIME_OFF:
        action = 'CHIME_OFF';
        break;
      default:
        this.log.warn(`XGenAlarm: unsupported chime state value ${value}`);
        return;
    }
  ...
  applyState() {
    ...
    // ALARM_TRIGGERED (HAP value 4) has no TargetState equivalent, so we
    // leave the target alone while triggered rather than send HomeKit an
    // invalid value — it already reflects whatever mode the area was
    // armed in right before it tripped, which is still the right thing
    // for HomeKit to show.
    if (!isTriggered) {
      this.lastTargetState = this.lastCurrentState;
    }

    if (this.currentStateChar) this.currentStateChar.updateValue(this.lastCurrentState);
    if (this.targetStateChar && this.lastTargetState !== undefined) {
      this.targetStateChar.updateValue(this.lastTargetState);
    }
  }

  /**
   * Sends the "alarm triggered" push notification (ntfy,
