...
class XGenClient {
   ...
   * @param {number} [options.fnumChimeOn=10] - fnum for Chime On.
   * @param {number} [options.fnumChimeOff=11] - fnum for Chime Off.
  ...
  constructor(options, log) {
    ...
    // Function numbers used by /user/keyfunction.cgi for chime on/off 
    // — overridable in config in case a firmware variant differs from the confirmed one.
    this.fnumChimeOn = options.fnumChimeOn != null ? options.fnumChimeOn : 10;
    this.fnumChimeOff = options.fnumChimeOff != null ? options.fnumChimeOff : 11;
  ...
  async _authedPost(path, extra, { allowDuringBackoff = false } = {}) {
    ...
    if (this._isSessionExpired(res)) {
      ...
      if (ageMs < this.sessionContentionThresholdMs) {
        ...
        // 
        // The best way to mitigate this entirely is to create a custom user
        // just for the plugin, and never log into the panel's own web UI with that user. 
        // That way the plugin's session is never contested, and it can poll freely without ever hitting this backoff.
  ...
  /**
   * Sends an action command for a single (1-based)
   ...
   * @param {'AWAY_ARM'|'STAY_ARM'|'DISARMED'|'CHIME_ON'|'CHIME_OFF'} action
  ...
  async sendAreaCommand(action, areaNumber) {
    const fnumMap = { AWAY_ARM: this.fnumAway, STAY_ARM: this.fnumStay, DISARMED: this.fnumDisarm, CHIME_ON: this.fnumChimeOn, CHIME_OFF: this.fnumChimeOff };
    const fnum = fnumMap[action];
    if (fnum == null) {
      throw new Error(`Unsupported action "${action}" — this panel's web UI only exposes Away/Stay/Disarm/Chime On/Chime Off.`);
    }
   ...
   *   loaded yet (e.g. before the first login/poll completes). `night` is
   *   still decoded here for diagnostic completeness (it's a real panel
   *   bit), even though deriveState() below no longer treats it as a
   *   distinct HomeKit-facing state — see deriveState()'s doc comment.
   */
   ...
   * a single area. Alarm takes priority over everything else, then
   * armed-away over armed-stay. The panel's "night" sub-mode of
   * armed-stay (decodeAreaState()'s `night` bit) isn't surfaced as a
   * distinct state here — there's no confirmed way to *request* Night
   * through this plugin (see the "Arm / Disarm" fnum list in the class
   * doc comment above; only Away/Stay/Disarm have confirmed fnums), so
   * it's folded into plain STAY_ARM rather than exposing a HomeKit state
   * nothing can actually arm into.
   * @param {number} areaNumber - 1-based.
  ...
  deriveState(areaNumber) {
    ...
    if (bits.armedStay) return 'STAY_ARM';
    return 'DISARMED';
  }
...
XGenClient.STATES = ['DISARMED', 'STAY_ARM', 'AWAY_ARM', 'ALARM_TRIGGERED'];
