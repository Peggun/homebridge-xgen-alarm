 ...
 *   Either case triggers an automatic re-login + one retry — see
...
class XGenClient {
  ...
  async sendAreaCommand(action, areaNumber) {
    ...
    const body = await this._authedPost('/user/keyfunction.cgi', `fnum=${fnum}&start=${start}&mask=${mask}`);
